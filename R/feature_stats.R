...
#' Summarize and Analyze Motif Feature Statistics
#'
#' @description
#' Legacy wrapper that forwards arguments to the default method of \code{\link{feature_stats}}.
#'
#' @param stats_df A data frame returned by \code{sap$features$motif$stats_fund_freq}
#'   or \code{sap$features$motif$stats_wiener_entropy}.  Must contain at minimum
#'   the columns \code{label}, \code{segment_id}, \code{mean}, \code{sd},
#'   \code{n_samples}, and \code{duration}.
#' @param feature_name Character string used as the y-axis label and plot title.
#'   Defaults to \code{"Feature"}.
#' @param labels Optional character vector of labels to include.
#' @param run_anova Logical. Whether to run ANOVA (default: \code{TRUE}).
#' @param plot Logical. Whether to draw a boxplot (default: \code{TRUE}).
#' @param plot_type Character. One of \code{"both"}, \code{"boxplot"}, or \code{"violin"}.
#' @param palette Character. RColorBrewer palette name (default: \code{"Set1"}).
#' @param jitter Logical. Add jittered individual-rendition points (default: \code{TRUE}).
#' @param point_alpha Numeric \[0, 1\]. Transparency of jittered points (default: \code{0.35}).
#' @param facet_segments Logical. Facet the plot by \code{segment_id} (default: \code{FALSE}).
#' @param ncol Integer. Number of columns when \code{facet_segments = TRUE}.
#'
#' @return A list (invisibly) as described in \code{\link{feature_stats}}.
#'
#' @export
sap_feature_stats <- function(stats_df,
                               feature_name   = "Feature",
                               labels         = NULL,
                               run_anova      = TRUE,
                               plot           = TRUE,
  ...
  feature_stats(
    ...
    facet_segments = facet_segments,
    ncol           = ncol
  )
}

#' Convenience wrapper: run sap_feature_stats directly from a SAP object
#'
#' @description
#' Legacy wrapper that forwards arguments to the \code{Sap} method of \code{\link{feature_stats}}.
#'
#' @param x A \code{Sap} object.
#' @param feature Character.  One of \code{"fund_freq"} (default) or
#'   \code{"wiener_entropy"}.  Partial matching is supported.
#' @param segment_type Character.  One of \code{"motifs"} (default),
#'   \code{"syllables"}, or \code{"segments"}.
#' @param ... Additional arguments forwarded to \code{\link{feature_stats}}.
#'
#' @return Invisibly, the list returned by \code{\link{feature_stats}}.
#'
#' @export
sap_stats <- function(x,
                      feature      = c("fund_freq", "wiener_entropy"),
                      segment_type = c("motifs", "syllables", "segments"),
                      ...) {
