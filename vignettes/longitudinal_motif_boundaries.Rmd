...
For large longitudinal datasets, boundary refinement and boundary heatmaps can
be run on a controlled subset of motifs. Use `labels` or `clusters` to focus on
specific groups, `sample_percent` to refine or plot a percentage from each
label, `balanced = TRUE` to draw equal counts across labels, or
`max_samples_per_label` to cap the number of randomly selected motifs per
label.

For example, this refines at most 100 motifs from each label while leaving
unselected motifs in `sap$motifs` with missing boundary values:

```{r sampling_controls, eval = FALSE}
sap <- refine_motif_boundaries(
  sap,
...
When `balanced = TRUE`, `max_samples_per_label` is ignored because balanced
sampling determines the per-label count automatically.
