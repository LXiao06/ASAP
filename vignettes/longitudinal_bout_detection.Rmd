...
will not add motif-bout relationship columns (n_motifs, align_time, etc.). Additionally, 
because template matching is not performed, it is possible to detect false positives 
such as innate vocalizations or background noise.
...
across song developmental stages.
...
![Bout Amplitude Envelope Heatmap](figures/longitudinal_bout_heatmap.png){width=100%}
...
**Summary of Vocal Bout Changes**:
- Bouts become progressively shorter and contain fewer motifs from BL to Post to Rec.
- The timing between bouts remains unchanged across stages.
...
For a full walkthrough of export arguments and additional scenarios, see the
[Exporting Curated Song Clips](exporting_song_clips.html) vignette.
