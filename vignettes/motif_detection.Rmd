...
Once you are satisfied with the detected motif boundaries, you can export each
motif as its own WAV file. This is useful for manual review, sharing examples,
or preparing a small set of motif clips for downstream analysis.

### Step 1: Create a temporary export directory

```{r export-motif-setup}
...
`create_motif_clips()` reads the detected motif table and writes one WAV file
per motif. For this example recording, the three detected motifs are small
enough that we can export all of them.

```{r export-motif-clips}
...
The returned metadata table tracks the original motif boundaries and the output
path of each generated file.

```{r export-motif-metadata}
...
Because this example only contains three motifs, we can inspect every exported
clip directly.

```{r visualize-exported-motifs, fig.height=4, fig.show='hold'}
...
```{r cleanup-exported-motifs, include = FALSE}
if (exists("motif_export_dir") && dir.exists(motif_export_dir)) {
  unlink(motif_export_dir, recursive = TRUE, force = TRUE)
}
```
...
| 8 | `create_motif_clips()` | Export detected motifs as WAV clips |
