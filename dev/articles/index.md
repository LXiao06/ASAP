# Get Started

ASAP (Automated Sound Analysis Pipeline) provides an integrated toolkit
for processing, analysing, and visualising avian vocalizations — from
single audio files to large longitudinal datasets. Choose a tutorial
below to get started.

Single WAV File Analysis

Learn ASAP functions using individual audio files.

[](https://lxiao06.github.io/ASAP/dev/articles/single_wav_analysis.md)

![Overview vignette showing spectrogram-based audio
exploration](figures/basic_audio_analysis.png)

Overview: ASAP 101

Load a WAV file, visualise spectrograms, detect bouts, segment
syllables, and export an example bout clip.

Beginner · ~10 min

[](https://lxiao06.github.io/ASAP/dev/articles/motif_detection.md)

![Motif detection spectrogram](figures/motif_detection.png)

Motif Detection

Build a cross-correlation template, detect motif occurrences in a single
recording, and review the detected segments.

Beginner · ~15 min

[](https://lxiao06.github.io/ASAP/dev/articles/acoustic_feature_analysis.md)

![Spectral entropy analysis plot](figures/acoustic_feature_analysis.png)

Acoustic Feature Analysis

Measure spectral entropy, extract pitch contours, and compare
amplitude-envelope structure from individual song segments.

Beginner · ~10 min

[](https://lxiao06.github.io/ASAP/dev/articles/advanced_acoustic_feature_analysis.md)

![Feature trajectory plots across motif
renditions](figures/acoustic_feature_analysis.png)

Advanced Acoustic Feature Analysis

Detect motifs, define a syllable window of interest, and extract
entropy, pitch, and amplitude features across all renditions for
statistical analysis.

Intermediate · ~20 min

Longitudinal Recording Analysis with SAP Object

Batch processing of recordings across developmental time points.

[](https://lxiao06.github.io/ASAP/dev/articles/construct_sap_object.md)

![ASAP package logo](figures/logo.png)

Constructing a SAP Object

Organise multi-day recording directories into a SAP object for
reproducible longitudinal analysis.

Intermediate · ~10 min

[](https://lxiao06.github.io/ASAP/dev/articles/longitudinal_motif_detection.md)

![Longitudinal motif heatmap across
development](figures/longitudinal_heatmap.png)

Longitudinal Motif Detection

Detect and align motifs across developmental time points. Visualise
acoustic changes with amplitude heatmaps and UMAP embeddings.

Intermediate · ~25 min

[](https://lxiao06.github.io/ASAP/dev/articles/longitudinal_bout_detection.md)

![Longitudinal bout amplitude
heatmap](figures/longitudinal_bout_heatmap.png)

Longitudinal Bout Detection

Detect song bouts across recordings, summarise motif-bout relationships,
and track changes in bout duration and density over development.

Intermediate · ~20 min

[](https://lxiao06.github.io/ASAP/dev/articles/exporting_song_clips.md)

![Conceptual illustration of audio motif and bout
extraction](figures/exporting_clips_card.png)

Exporting Curated Song Clips

Generate precisely cropped motif or bout-level audio files (WAV or HDF5)
from raw recordings for use in external processing pipelines.

Intermediate · ~10 min

[](https://lxiao06.github.io/ASAP/dev/articles/longitudinal_syllable_segmentation.md)

![UMAP of syllable segments coloured by
cluster](figures/longitudinal_syllable_umap.png)

Longitudinal Syllable Segmentation

Segment detected bouts into individual syllables, extract spectral
features, cluster segments, and visualise the acoustic structure with
UMAP dimensionality reduction.

Advanced · ~25 min

[](https://lxiao06.github.io/ASAP/dev/articles/syllable_labeling.md)

![Syllable cluster heatmap after
labeling](figures/longitudinal_syllable_manual_heatmap.png)

Syllable Labeling

Assign letter identities to syllable clusters using automatic
DBSCAN-based labeling and manual refinement. Verify the final syllable
inventory across developmental stages.

Advanced · ~20 min

[](https://lxiao06.github.io/ASAP/dev/articles/longitudinal_motif_boundaries.md)

![Amplitude-envelope heatmap with onset (green) and offset (cyan)
boundary markers](figures/motif_boundaries_heatmap.png)

Identifying Motif Boundaries

Refine coarse template windows into precise motif onset/offset times
using underlying syllable segments. Visualise boundaries with annotated
heatmaps and verify alignment across developmental stages.

Advanced · ~20 min

[](https://lxiao06.github.io/ASAP/dev/articles/longitudinal_motif_trajectory.md)

![UMAP of motif trajectory embeddings coloured by developmental
label](figures/motif_trajectory_umap.png)

Longitudinal Motif Trajectory Analysis

Build trajectory matrices from spectral embeddings, reduce with PCA and
UMAP, and visualise how motif acoustic structure shifts across
developmental stages.

Advanced · ~25 min

[](https://lxiao06.github.io/ASAP/dev/articles/longitudinal_motif_similarity.md)

![Per-label trajectory similarity distributions
(Correlation)](figures/motif_trajectory_similarity_card.png)

Longitudinal Motif Similarity Analysis

Filter outlier renditions, compute per-label trajectory similarity to a
baseline reference using RMS, Fréchet, DTW, and correlation metrics, and
visualise how acoustic convergence changes across developmental stages.

Advanced · ~20 min
