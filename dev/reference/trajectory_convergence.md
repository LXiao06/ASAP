...
Fr\\echet, DTW) with unified variability scaling and correlation
analysis.

## Usage

``` r
trajectory_convergence(x, ...)
...
- metrics:
  ...
  Character vector of metrics to compute (default: c("rms", "frechet",
  "dtw", "correlation"))
...
- `type`: Character string `"convergence"`
...
`x$features[[feature_type]][["trajectory_convergence"]]`
...
defines the target, not a convergence point).
