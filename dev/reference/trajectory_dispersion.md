...
Quantifies trajectory dispersion across experimental conditions using
three complementary metrics computed in PCA or UMAP embedding space.

## Usage

``` r
trajectory_dispersion(x, ...)
...
- `type`: Character string `"dispersion"`, used by
...
`x$features[[feature_type]][["trajectory_dispersion"]]` (returned
invisibly).
